...
import { BuilderMode, MonsterType, MonsterAttributes, NDTable, NDKey, SaveType } from './types';
...
export class MonsterBuilder {
     ...
     */
    private getTable(type: MonsterType): NDTable {
        switch (type) {
     ...
     */
    generateMonsterFromTemplate(nd: string, type: MonsterType): MonsterAttributes {
     ...
     */
    calculateFinalND(attributes: MonsterAttributes): number {
     ...
     */
    async createMonsterActor(data: MonsterAttributes & { name: string; type: MonsterType }): Promise<Actor> {
        ...
        return await Actor.create(actorData as any) as Actor;
    }
}
 ...
 */
export function validateMonsterStats(stats: MonsterAttributes): boolean {
 ...
 */
export function calculateMonsterStats(nd: NDKey, type: MonsterType): MonsterAttributes {
 ...
 */
export function getAdjustedSave(stats: MonsterAttributes, saveType: SaveType): number {
    switch (saveType) {
        case 'Forte':
 ...
 */
export function applyTraitModifiers(stats: MonsterAttributes, traits: string[]): MonsterAttributes {
 ...
 */
export function formatMonsterStats(stats: MonsterAttributes): string {
    return `
        ND: ${stats.nd}
        PV: ${stats.pv}
        Defesa: ${stats.defesa}
